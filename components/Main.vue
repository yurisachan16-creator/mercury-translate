<template>
  ...
  <div v-show="config.on" class="settings-main-sections">
    ...
    <section v-show="props.activeSection === 'settings-data'" id="settings-data" class="settings-section data-section">
        ...
        <el-row class="margin-bottom margin-left-2em config-transfer-secret-choice">
          <el-col :span="24">
            <el-checkbox v-model="includeExportSecrets">{{ t('main.includeSecretsInExport') }}</el-checkbox>
            <p v-if="includeExportSecrets" class="config-transfer-secret-warning">{{ t('main.includeSecretsWarning') }}</p>
            <p v-else class="config-transfer-safe-boundary">{{ t('main.safeExportBoundary') }}</p>
          </el-col>
        </el-row>
        ...
        <el-row v-if="showImportBox" class="margin-bottom margin-left-2em">
          <el-col :span="24">
            <el-input v-model="importData" type="textarea" :rows="8" :placeholder="t('main.pasteJsonConfig')" @input="clearImportPreview" />
            <section v-if="importPreview" class="config-transfer-preview" aria-live="polite">
              <strong>{{ t('main.importPreviewTitle') }}</strong>
              <span>{{ importPreview.legacy ? t('main.importPreviewLegacy') : `${t('main.importPreviewVersion')}: ${importPreview.appVersion}` }}</span>
              <span>{{ importPreview.secretsIncluded ? t('main.importPreviewSecretsIncluded') : t('main.importPreviewSecretsExcluded') }}</span>
            </section>
            <div style="margin-top: 10px; text-align: right;">
              <el-button @click="previewImport">{{ t('main.previewImport') }}</el-button>
              <el-button type="primary" :disabled="!importPreview" @click="saveImport">{{ t('main.importConfig') }}</el-button>
            </div>
          </el-col>
        </el-row>
    </section>
...
const includeExportSecrets = ref(false);
const showImportBox = ref(false);
const importData = ref('');
const importPreview = ref<SettingsTransferPreview | null>(null);
...
// A previously generated plaintext-secret envelope must not remain visible
// after the user returns to the safe-export setting.
watch(includeExportSecrets, () => {
  exportData.value = '';
...
const handleExport = async () => {
  try {
    await configReady;
    if (includeExportSecrets.value) {
      await ElMessageBox.confirm(
        t('main.includeSecretsConfirmMessage'),
        t('main.includeSecretsConfirmTitle'),
        {confirmButtonText: t('main.confirm'), cancelButtonText: t('main.cancel'), type: 'warning'},
      );
    }
...
const handleImport = () => {
  ...
  showExportBox.value = false;
  if (!showImportBox.value) clearImportPreview();
};
...
const importErrorMessage = (error: unknown) => {
  if (error instanceof SettingsTransferValidationError && error.code === 'unsupported-schema') {
    return t('main.importUnsupportedSchema');
  }
  return t('main.invalidConfigFormatCheck');
};

const previewImport = () => {
  try {
    const parsedConfig = JSON.parse(importData.value);
    importPreview.value = parseSettingsTransfer(parsedConfig);
    ElMessage({
      message: t('main.importPreviewReady'),
      type: 'success',
    });
...
const saveImport = async () => {
  ...
  try {
    await ElMessageBox.confirm(
      preview.secretsIncluded ? t('main.importSecretsConfirmMessage') : t('main.importConfirmMessage'),
      t('main.importConfirmTitle'),
      {confirmButtonText: t('main.confirm'), cancelButtonText: t('main.cancel'), type: 'warning'},
    );

    // Force a distinct, restorable local-history snapshot before applying a
    // transfer from a different extension ID. The transfer content is never
    // logged.
    await createConfigBackup();
    await persistConfig(normalizeConfig(preview.config));
    ElMessage({
    ...
    // Element Plus rejects the confirmation promise for a normal cancellation.
    if (error === 'cancel' || error === 'close') return;
    ElMessage({
      message: `${t('main.importFailedPrefix')}${error instanceof Error ? error.message : t('main.tryAgainLater')}`,
      type: 'error',
    });
  }
};
...
.config-transfer-secret-choice {
  margin-top: -8px;
}

.config-transfer-secret-warning {
  margin: 6px 0 0;
  color: #b42318;
  font-size: 12px;
  line-height: 1.5;
