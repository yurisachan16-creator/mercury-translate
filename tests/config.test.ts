...
describe('统一配置存储', () => {
    ...
    it('导入前备份即使配置未变化也保留独立可恢复快照', async () => {
        const configStore = await loadConfigModule(storedConfig);
        await Promise.all([configStore.configReady, configStore.configHistoryReady]);

        const before = configStore.getConfigHistorySnapshot();
        await configStore.createConfigBackup();
        const after = configStore.getConfigHistorySnapshot();

        expect(after.entries).toHaveLength(before.entries.length + 1);
        expect(after.entries.at(-1)?.config).toMatchObject(storedConfig);
        expect(after.entries.at(-1)?.version).not.toBe(before.entries.at(-1)?.version);
    });
