...
async function translatePage(
  ...
  try {
    translations = await translateBlocks(pageIndex, blocks, signal);
  } catch (error) {
    if (isNetworkConsentRequiredError(error)) scheduler?.cancel();
    throw error;
  }
...
function resetTranslations(): void {
  ...
  pages.value.forEach(page => setPageState(page.pageIndex, {status: 'idle', error: undefined}));
  configureScheduler();
