...
import { isNetworkConsentRequiredError } from '@/entrypoints/utils/providerConsent';
...
export function shouldWarnVideoSubtitleTranslationFailure(error: unknown): boolean {
  if (isNetworkConsentRequiredError(error)) return false;
  if (error instanceof Error && (error.name === 'NetworkConsentRequiredError' || error.name === 'AbortError')) return false;
  return true;
}
...
function isYouTubeHostPage(locationLike: Pick<Location, 'hostname'> = window.location): boolean {
  return YOUTUBE_HOST_PATTERN.test(locationLike.hostname) || YOUTUBE_MOBILE_HOST_PATTERN.test(locationLike.hostname);
}
...
export function mountVideoSubtitleTranslation(): () => void {
  if (!isYouTubeHostPage()) return () => undefined;
  ...
  let videoPageActive = isYouTubeVideoPage();
  ...
  const updateProgressiveCaption = (source: string, overlay: HTMLElement, container: HTMLElement): boolean => {
    ...
    void getCachedVideoTranslation(cue.text).then((translated) => {
      ...
      if (!destroyed && requestGeneration === generation && shouldWarnVideoSubtitleTranslationFailure(error)) {
  ...
  const syncPlayerUi = () => {
    ...
    const nextVideoPageActive = isYouTubeVideoPage();
    if (nextVideoPageKey !== videoPageKey || nextVideoPageActive !== videoPageActive) {
      videoPageKey = nextVideoPageKey;
      videoPageActive = nextVideoPageActive;
      captionObserver?.disconnect();
      captionObserver = undefined;
      observedContainer = null;
