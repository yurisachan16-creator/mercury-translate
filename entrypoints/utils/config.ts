...
async function appendHistorySnapshotNow(value: unknown, force = false): Promise<void> {
    ...
    if (!force && current && serializeConfig(current.config) === serializeConfig(normalized)) return;
...
/**
 * Preserve a distinct, immediately restorable snapshot before a destructive
 * user-initiated operation such as importing settings from another extension
 * ID. Unlike normal saves, this intentionally keeps an equal-value snapshot
 * so the pre-import checkpoint cannot be coalesced away.
 */
export async function createConfigBackup(value: unknown = config): Promise<void> {
    await configHistoryReady;
    await flushConfigHistory();
    await appendHistorySnapshotNow(value, true);
}
