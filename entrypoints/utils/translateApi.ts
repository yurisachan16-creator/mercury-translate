...
const sessionNetworkConsentDenials = new Set<string>();

interface SessionNetworkConsentRequest {
  activeCallers: number;
  abortedByAllCallers: boolean;
  controller: AbortController;
  promise: Promise<string>;
  settled: boolean;
}

const sessionNetworkConsentRequests = new Map<string, SessionNetworkConsentRequest>();
...
async function requestConsentProviderOverride(
  ...
  const denialKey = getNetworkConsentDenialKey(error, configuredService);
  if (sessionNetworkConsentDenials.has(denialKey)) throw error;
  const request = getOrCreateNetworkConsentRequest(denialKey, error);
  return waitForNetworkConsentRequest(request, signal);
}
...
function getNetworkConsentDenialKey(error: NetworkConsentRequiredError, configuredService: string): string {
  const providerKey = error.outcome.providerId || error.outcome.availableProviders.join(',');
  return `${configuredService}:${error.outcome.reason}:${providerKey}`;
}

function getOrCreateNetworkConsentRequest(
  denialKey: string,
  error: NetworkConsentRequiredError,
): SessionNetworkConsentRequest {
  const current = sessionNetworkConsentRequests.get(denialKey);
  if (current) return current;

  const request: SessionNetworkConsentRequest = {
    activeCallers: 0,
    abortedByAllCallers: false,
    controller: new AbortController(),
    promise: Promise.resolve(''),
    settled: false,
  };
  request.promise = (async () => {
    const decision = await requestNetworkProviderConsent(error.outcome, request.controller.signal);
    if (!decision) {
      if (request.abortedByAllCallers) throw createAbortError();
      sessionNetworkConsentDenials.add(denialKey);
      throw error;
    }

    sessionNetworkProviderOverride = decision.providerId;
    sessionNetworkConsentDenials.delete(denialKey);
    return decision.providerId;
  })().finally(() => {
    request.settled = true;
    if (sessionNetworkConsentRequests.get(denialKey) === request) {
      sessionNetworkConsentRequests.delete(denialKey);
    }
  });

  sessionNetworkConsentRequests.set(denialKey, request);
  return request;
}

function waitForNetworkConsentRequest(
  request: SessionNetworkConsentRequest,
  signal?: AbortSignal,
): Promise<string> {
  throwIfAborted(signal);
  request.activeCallers += 1;
  ...
  const release = () => {
    ...
    released = true;
    request.activeCallers -= 1;
    if (request.activeCallers === 0 && !request.settled) {
      request.abortedByAllCallers = true;
    ...
    const translationTask = async (
      ...
      try {
        ...
        if (isNetworkConsentRequiredError(error)) {
          ...
          const consentedProvider = await requestConsentProviderOverride(error, config.service, signal);
  ...
  return enqueueTranslation(async (lease) => {
    const translate = async (
      ...
      try {
        ...
        if (isNetworkConsentRequiredError(error)) {
          if (consentAttempted) throw error;
          const consentedProvider = await requestConsentProviderOverride(error, config.videoService);
...
export function cancelAllTranslations() {
  ...
  sessionNetworkConsentDenials.clear();
  sessionNetworkConsentRequests.forEach((request) => {
    request.abortedByAllCallers = true;
    request.controller.abort();
  });
  sessionNetworkConsentRequests.clear();
  clearTranslationQueue();
